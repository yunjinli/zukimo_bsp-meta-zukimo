...
# try to find git
if(VERSION_UPDATE_FROM_GIT AND NOT GIT_FOUND)
    include(FindGit)
endif()

# Check if git is found...
if (GIT_FOUND AND VERSION_UPDATE_FROM_GIT)
	# find last git tag of the form "^releases/v"
	...
	while(TRUE)
		if("${tag}" MATCHES "^releases/v")
			set(${PROJECT_NAME}_VERSION_STRING "${tag}")
			break()
		endif()

		execute_process(COMMAND ${GIT_EXECUTABLE} describe --abbrev=0 --tags "${tag}^"
			WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
			OUTPUT_VARIABLE tag
			OUTPUT_STRIP_TRAILING_WHITESPACE)
	endwhile()

	# How many commits since version tag
	...
	# Remove branch path name till first slash if existing
	string(REGEX REPLACE "[a-z]*\/" "" ${PROJECT_NAME}_VERSION_STRING ${${PROJECT_NAME}_VERSION_STRING})
...
elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/VERSION)
	...
	# neither git nor VERSION file available - print warning and continue
	message(NOTICE "Could not determine version (git history and VERSION file not present)")

	set(${PROJECT_NAME}_VERSION_STRING_FULL "unknown")
	set(${PROJECT_NAME}_VERSION_STRING "unknown")
...
add_definitions( -D${PROJECT_NAME}_VERSION_STRING_FULL=${${PROJECT_NAME}_VERSION_STRING_FULL} )
add_definitions( -D${PROJECT_NAME}_VERSION_STRING=${${PROJECT_NAME}_VERSION_STRING} )
...
add_definitions( -D${PROJECT_NAME}_VERSION_TWEAK=${${PROJECT_NAME}_VERSION_TWEAK} )
add_definitions( -D${PROJECT_NAME}_VERSION_AHEAD=${${PROJECT_NAME}_VERSION_AHEAD} )
add_definitions( -D${PROJECT_NAME}_VERSION_GIT_SHA=${${PROJECT_NAME}_VERSION_GIT_SHA} )
